 ...
 * using the integer primitives as a source of randomness. Some of the methods tested assume that
 * the range is known and constant (i.e. can benefit from a pre-computation stage).
 */
...
public class IntegerInRangeGenerationPerformance {
    /**
     * The number of sample values to generate.
     *
     * <p>Set this to 1 to observe the effect of the pre-computation stage.</p>
     */
    @Param({
            //"1",
            //"100",
    ...
    public static class IntRange {
         ...
         * The upper range for the {@code int} generation.
         *
         * <p>Note that the nextInt while loop uses a rejection algorithm. From the Javadoc for
         * java.util.Random:</p>
         *
         * <pre>
    ...
    /**
     * The powers of 2 to test for the fast {@code int} generation.
     */
    @State(Scope.Benchmark)
    public static class IntPowerRange {
        /**
         * The upper range for the {@code int} generation.
         */
        @Param({
     ...
     * (exclusive) using a bit source of randomness.
     *
     * <p>This method produces non-uniform samples when n is not a power of 2.</p>
     *
     * @param rng Value to use as a source of randomness.
     * @param n Bound on the random number to be returned. Must be positive.
     * @return a random {@code int} value between 0 (inclusive) and {@code n}
     ...
     */
    @Benchmark
    public void nextIntMask(Blackhole bh, Sources source, IntPowerRange range) {
        final int n = range.getUpperBound();
        final int mask = n - 1;
        for (int i = loops; i-- != 0; ) {
            bh.consume(source.getGenerator().nextInt() & mask);
        }
    }
     ...
     */
    @Benchmark
    public void nextIntShift(Blackhole bh, Sources source, IntPowerRange range) {
        final int n = range.getUpperBound();
        // Assumes n > 0
        final int shift = Integer.numberOfLeadingZeros(n) + 1;
        for (int i = loops; i-- != 0; ) {
            bh.consume(source.getGenerator().nextInt() >>> shift);
        }
    }
}
